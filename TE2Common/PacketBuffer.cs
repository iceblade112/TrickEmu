...
namespace TE2Common
{
    public class PacketBuffer
    {
        ...
        public PacketBuffer(Socket sock = null)
        {
        ...
        /// </summary>
        public byte[] getPacketDecrypted(bool withHeader = true)
        {
            ...
            if (withHeader) {
                ...
                // Packet header
                send.AddRange(header.ToArray());
            }

            send.AddRange(packet.ToArray());

            return send.ToArray();
        }

        public void Send(bool encrypt = true)
        {
            if(!encrypt)
            {
                socket.Send(this.getPacketDecrypted());
                return;
            }
        ...
        public void WriteUshort(ushort us, bool flip = false)
        {
            if(flip)
            {
        ...
        {
            packet.AddRange(Encoding.GetEncoding("gb2312").GetBytes(str));
            return;
        }
        ...
        {
            WriteByteArray(Encoding.GetEncoding("gb2312").GetBytes(str));
            if (str.Length < len)
            {
