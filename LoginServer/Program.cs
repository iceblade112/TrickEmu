    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                _serverSocket.Bind(new IPEndPoint(IPAddress.Any, int.Parse(config.Server["LoginPort"])));
                _serverSocket.Listen(5);
                _serverSocket.BeginAccept(AcceptCallback, null);
                logger.Info("Server has been started on port {0}.", config.Server["LoginPort"]);
            }
            catch (Exception ex)
            {
