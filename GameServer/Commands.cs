...
namespace TrickEmu
{
    class Commands
    {
        public static bool Handle(Socket sock, string chat)
        {
            ...
            switch (command.Substring(1))
            {
                ...
                case "move":
                    ...
                    if (param.Length < 2)
                    {
                        GameFunctions.SendChat("Usage: !move [Map ID]", sock);
                        return true;
                    }
                    ...
                    try
                    {
                        GameFunctions.SendChat("Changing zone to " + Program.Maps.MapName[MapId] + ".", sock);
                    }
                    catch
                    {
                        GameFunctions.SendChat("Changing zone to ID " + MapId + " without position data.", sock);
                    }
                    ...
                    if (param.Length > 2)
                    {
                        ...
                        {
                            GameFunctions.SendChat("Invalid X position given. Using default X 512.", sock);
                    ...
                    else
                    {
                        try
                        {
                            pkt2.WriteUshort((ushort)Program.Maps.MapPos[MapId].X); // X pos
                            PosX = Program.Maps.MapPos[MapId].X;
                        }
                        catch
                        {
                            pkt2.WriteUshort((ushort)PosX);
                    ...
                    if (param.Length > 3)
                    {
                        ...
                        {
                            GameFunctions.SendChat("Invalid Y position given. Using default Y 512.", sock);
                    ...
                    else
                    {
                        try
                        {
                            pkt2.WriteUshort((ushort)Program.Maps.MapPos[MapId].Y); // Y pos
                            PosY = Program.Maps.MapPos[MapId].Y;
                        }
                        catch
                        {
                            pkt2.WriteUshort((ushort)PosY);
