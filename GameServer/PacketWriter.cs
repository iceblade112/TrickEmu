...
namespace TrickEmu
{
    class PacketWriter
    {
        ...
        {
            System.Threading.Thread.Sleep(10); // TO-DO: Remove?
        ...
        public static void Chat(byte[] dec, Socket sock)
        {
            ...
            if (chatString.StartsWith("!gmc "))
            {
                ...
                PacketBuffer header = new PacketBuffer();
                header.WriteUshort(0x13C); // Packet ID
                header.WriteByteArray(new byte[] { 0x00, 0x00, 0x01 }); // Packet header padding
            ...
            else if(chatString.StartsWith("!oxc "))
            {
                // Normal chat
                PacketBuffer header = new PacketBuffer();
                header.WriteByte(0xB5); // Packet ID for O/X chat
                header.WriteByteArray(new byte[] { 0x00, 0x00, 0x00, 0x01 });
                ...
                Console.WriteLine("Sending to " + Program._clientSockets.ToArray().Length + " clients.");
            ...
            else
            {
                // Normal chat
                PacketBuffer header = new PacketBuffer();
                header.WriteByte(0x39); // Packet ID
                header.WriteByteArray(new byte[] { 0x00, 0x00, 0x00, 0x01 });
                ...
                byte[] newpkt = Encryption.encrypt(header.getPacket(false), data.getPacket());
